...
documentation without crawling the rendered site. Outputs land in docs/extra/
which Sphinx copies verbatim to the build root via ``html_extra_path``.
"""
...
from inspect import cleandoc, getdoc, getfullargspec, getmembers, isclass, signature
...
sys.path.append(".")
from docs.utils import including_sub_modules, remove_dups  # noqa: E402
...
# Curated pages with summaries. Ordering mirrors docs/index.rst toctree.
USING_PAGES = [
...
PYINFRA_DIR = REPO_ROOT / "src" / "pyinfra"
OUT_DIR = DOCS_DIR / "extra"
...
def render_operation_section(module_name: str) -> list[str]:
    ...
    lines.append("")
    if module.__doc__:
        lines.append(cleandoc(module.__doc__))
        lines.append("")
    ...
    for name, func in _extract_operation_funcs(module):
        ...
        if getattr(func, "is_idempotent", None) is False:
            ...
            lines.append("")
        if func.__doc__:
            lines.append(cleandoc(func.__doc__))
            lines.append("")
...
def render_fact_section(module_name: str) -> list[str]:
    ...
    for fact_name, cls in _extract_fact_classes(module):
        ...
        lines.append("")
        if cls.__doc__:
            lines.append(cleandoc(cls.__doc__))
            lines.append("")
