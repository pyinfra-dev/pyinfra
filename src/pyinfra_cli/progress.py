...
# cyan; files are a distinct grouping so they read as magenta.
_BAR_STYLE = {
    NodeKind.FILE: "magenta",
...
class Node:
    ...
    def __init__(
        ...
        # Verbose per-host lines (fact loads, command input/output, ...)
        # rendered nested under this node: list of (text, is_error).
        self.detail_lines: list[tuple[str, bool]] = []
    ...
    def add_detail_line(self, text: str, is_error: bool = False) -> None:
        if text:
            self.detail_lines.append((text, is_error))

    def succeed(self, detail: str | None = None) -> None:
    ...
    def render_rows(self, table: Table) -> None:
        ...
        if self.is_parent:
            ...
            row_detail = (
                Text(self.detail, style=_STATUS_STYLE.get(self.status, "dim"))
                if self.detail
                else Text("")
            )
        ...
        if self.detail_lines:
            ...
            if self.status == NodeStatus.RUNNING:
                lines = lines[-RUNNING_DETAIL_LINES:]
            detail_indent = "  " * (self.depth + 1)
            for text, is_error in lines:
                table.add_row(
                    Text(""),
                    Text(f"{detail_indent}{text}", style="red" if is_error else "dim"),
                    Text(""),
                    Text(""),
                )
