...
class TestSSHConnector(AsyncPatchSSHTestCase):
    ...
    async def test_connect_options(self):
        ...
        assert client.known_hosts_files == [str(Path("/does/not/exist/known_hosts").expanduser())]
    ...
    async def test_put_file_sudo_retries_freebsd_nfsv4_acl(self):
        state, host, connection = await self.connect_host(hosts=("anotherhost",))

        temp_file = "/tmp/pyinfra-de01e82cb691e8a31369da3c7c8f17341c44ac24"
        connection.add_response(
            1,
            stderr=[
                f"setfacl: {temp_file}: branding mismatch; "
                "existing ACL is NFSv4, entry to be merged is POSIX.1e",
            ],
        )
        ...
        assert status is True
        assert connection.commands == [
            f"sh -c 'setfacl -m u:root:r {temp_file}'",
            f"sh -c 'setfacl -m u:root:r::allow {temp_file}'",
            f"sudo -H -n -u root sh -c 'cp {temp_file} '\"'\"'not another file'\"'\"''",
            f"sh -c 'rm -f {temp_file}'",
        ]
