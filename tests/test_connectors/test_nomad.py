...
class TestNomadConnector(TestCase):
    def setUp(self):
        self.local_connect_patch = patch(
            "pyinfra.connectors.nomad.LocalConnector.connect",
            new_callable=AsyncMock,
        )
        self.local_connect = self.local_connect_patch.start()

        self.local_run_shell_patch = patch(
            "pyinfra.connectors.nomad.LocalConnector.run_shell_command",
            new_callable=AsyncMock,
        )
    ...
    def test_connect_all(self):
        inventory = make_inventory(hosts=("@nomad/alloc123/web",))
    ...
    def test_run_shell_command(self):
        ...
        host = inventory.get_host("@nomad/alloc123/web")
        host.connect()
