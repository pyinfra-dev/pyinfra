...
class TestPodmanConnector(TestCase):
    def setUp(self):
    ...
    def test_missing_image(self):
        with self.assertRaises(InventoryError):
            make_inventory(hosts=("@podman",))

    def test_user_provided_container_id(self):
        inventory = make_inventory(
            hosts=(("@podman/not-an-image", {"docker_container_id": "abc"}),),
        )
        State(inventory, Config())
        host = inventory.get_host("@podman/not-an-image")
        host.connect()
        assert host.data.docker_container_id == "abc"

    def test_connect_all(self):
        inventory = make_inventory(hosts=("@podman/not-an-image",))
        state = State(inventory, Config())
        connect_all(state)
        assert len(state.active_hosts) == 1

    def test_connect_all_error(self):
        inventory = make_inventory(hosts=("@podman/a-broken-image",))
        state = State(inventory, Config())
    ...
    def test_run_shell_command(self):
        ...
        podman_command = "podman exec -it containerid sh -c {0}".format(command)
        shell_command = make_unix_command(podman_command).get_raw_value()
    ...
    def test_put_file(self):
        ...
        self.fake_popen_mock.assert_called_with(
            "sh -c 'podman cp __tempfile__ containerid:not-another-file'",
            shell=True,
            stdout=PIPE,
            stderr=PIPE,
            stdin=PIPE,
    ...
    def test_get_file(self):
        ...
        self.fake_popen_mock.assert_called_with(
            "sh -c 'podman cp containerid:not-a-file __tempfile__'",
            shell=True,
            stdout=PIPE,
            stderr=PIPE,
            stdin=PIPE,
