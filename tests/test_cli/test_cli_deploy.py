...
class TestCliDeployState(PatchSSHTestCase):
    ...
    def test_deploy(self):
        ...
        correct_op_name_and_host_names = [
            ...
            (f"{a_task_file_path} | First task operation", ("anotherhost",)),
            (f"{a_task_file_path} | Task order loop 1", ("anotherhost",)),
            (f"{a_task_file_path} | 2nd Task order loop 1", ("anotherhost",)),
            (f"{a_task_file_path} | Task order loop 2", ("anotherhost",)),
            (f"{a_task_file_path} | 2nd Task order loop 2", ("anotherhost",)),
            (
                f"{a_task_file_path} | {nested_task_path} | Second task operation",
                ("anotherhost",),
            ),
            (f"{a_task_file_path} | First task operation", True),
            (f"{a_task_file_path} | Task order loop 1", True),
            (f"{a_task_file_path} | 2nd Task order loop 1", True),
            (f"{a_task_file_path} | Task order loop 2", True),
            (f"{a_task_file_path} | 2nd Task order loop 2", True),
            (f"{a_task_file_path} | {nested_task_path} | Second task operation", True),
            (f"{b_task_file_path} | Important task operation", True),
